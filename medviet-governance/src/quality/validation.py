...
def validate_anonymized_data(filepath: str) -> dict:
    ...
    # Dùng regex để tìm xem có ô nào chứa đúng 12 chữ số hay không
    if df["cccd"].astype(str).str.match(r"^\d{12}$").any():
        results["success"] = False
        results["failed_checks"].append("Data Leakage: CCCD cột vẫn chứa dữ liệu gốc 12 số.")
    ...
    for col in critical_columns:
        if col in df.columns and df[col].isnull().any():
            results["success"] = False
            results["failed_checks"].append(f"Missing Values: Cột '{col}' chứa giá trị null.")

    # Check 3: Số rows phải bằng original
    original_df = pd.read_csv("data/raw/patients_raw.csv")
    if len(df) != len(original_df):
        results["success"] = False
        results["failed_checks"].append(f"Row count mismatch: Anonymized ({len(df)}) vs Raw ({len(original_df)}).")
